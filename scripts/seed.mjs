...
CREATE TABLE IF NOT EXISTS results (id INTEGER PRIMARY KEY AUTOINCREMENT, task_code TEXT NOT NULL, kind TEXT NOT NULL, version TEXT NOT NULL, summary TEXT DEFAULT "", status TEXT NOT NULL DEFAULT "未接入", source_ref TEXT DEFAULT "", is_demo INTEGER NOT NULL DEFAULT 1);
...
insTask.run("FDE-SIM-01", "仿真演示接入（AspenSA 两工况 V0.3 / 葡萄糖三效热耦合 V0.2）", "V0.3/V0.2", "AtmEdge 产品主线", "待确认", "本地已有演示成果", "", "等待云边接口联调任务卡（第二阶段）", 1);
insTask.run("FDE-WEB-01", "工程工作台网站首版", "任务书 V0.1", "盖茨", "已提交", "网站开发与部署", "", "待项目总控复验（验收意见 V0.1 / WEB-R01—08）", 0);

const insRes = db.prepare("INSERT INTO results (task_code, kind, version, summary, status, source_ref, is_demo) VALUES (?,?,?,?,?,?,?)");
insRes.run("FDE-SIM-01", "H（工程师基准）", "—", "同题工程师工时数据未采集，效率比较显示 待测/N/A", "待测", "待核实（无采集记录）", 1);
insRes.run("FDE-SIM-01", "A1（智能体首次）", "—", "未接入：云边接口未实现，不显示假进度", "未接入", "待核实（接口未实现）", 1);
insRes.run("FDE-SIM-01", "C1（人工修正协同）", "—", "未接入：依赖 A1，A1 未产生前不生成", "未接入", "待核实（依赖 A1）", 1);
